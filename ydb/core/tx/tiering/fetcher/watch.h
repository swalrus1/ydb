...
class TEvWatchTieredStorages: public TEventLocal<TEvWatchTieredStorages, NTiers::EvWatchTieredStorages> {
private:
    YDB_READONLY_DEF(std::vector<TString>, Names);

public:
    TEvWatchTieredStorages(std::vector<TString> names)
        : Names(std::move(names)) {
    }
};

class TEvNotifyTieredStorageUpdated: public TEventLocal<TEvNotifyTieredStorageUpdated, NTiers::EvNotifyTieredStorageUpdated> {
private:
    YDB_READONLY_DEF(TString, Name);
    YDB_READONLY_DEF(NTiers::TTierConfig, Config);

public:
    TEvNotifyTieredStorageUpdated(const TString& name, NTiers::TTierConfig config)
        : Name(name)
        , Config(std::move(config)) {
    }
};

class TEvNotifyTieredStorageDeleted: public TEventLocal<TEvNotifyTieredStorageDeleted, NTiers::EvNotifyTieredStorageDeleted> {
private:
    YDB_READONLY_DEF(TString, Name);

public:
    TEvNotifyTieredStorageDeleted(TString name)
        : Name(std::move(name)) {
    }
};
...
class TEvTieredStorageResolutionFailed: public TBaseEvObjectResolutionFailed,
                                      public TEventLocal<TEvTieredStorageResolutionFailed, NTiers::EvTieredStorageResulutionFailed> {
public:
...
private:
    TActorId Owner;
    THashSet<TString> WatchedTieredStorages;

private:
    template <typename TObject>
    static TString GetStorageDirectory() {
        return JoinPath({AppDataVerified().TenantName, TObject::GetLocalStorageDirectory()});
    }

    void WatchTieredStorages(const std::vector<TString>& tieredStorages) {
        std::vector<TString> newTieredStorages;
        for (const TString& tieredStorageId : tieredStorages) {
            if (WatchedTieredStorages.emplace(tieredStorageId).second) {
    ...
    void OnPathNotFound(const TVector<TString>& path) {
        ...
        OnObjectResolutionFailure(path, NTiers::TEvTieredStorageResolutionFailed::EReason::NOT_FOUND);
    }
    ...
    void OnLookupError(const TVector<TString>& path) {
        ...
        OnObjectResolutionFailure(path, NTiers::TEvTieredStorageResolutionFailed::EReason::LOOKUP_ERROR);
    }

    void OnObjectResolutionFailure(const TVector<TString>& pathComponents, const NTiers::TEvTieredStorageResolutionFailed::EReason reason) {
        ...
        if (IsEqualPaths(storageDirectory, GetStorageDirectory<NTiers::TTierConfig>())) {
            WatchedTieredStorages.erase(objectId);
            Send(Owner, new NTiers::TEvTieredStorageResolutionFailed(objectId, reason));
        } else {
    ...
    void OnTieredStorageDeleted(const TString& name) {
        Send(Owner, new NTiers::TEvNotifyTieredStorageDeleted(name));
    }
...
public:
    ...
    void Handle(TEvTxProxySchemeCache::TEvWatchNotifyDeleted::TPtr& ev) {
        ...
        if (IsEqualPaths(storageDir, GetStorageDirectory<NTiers::TTierConfig>())) {
            WatchedTieredStorages.erase(name);
            Send(Owner, new NTiers::TEvNotifyTieredStorageDeleted(name));
        } else {
    ...
    void Handle(NTiers::TEvWatchTieredStorages::TPtr& ev) {
        ...
        WatchTieredStorages(ev->Get()->GetNames());
    }
    ...
    void Handle(NActors::TEvents::TEvUndelivered::TPtr& ev) {
        ...
        for (const TString& tieredStorageId : WatchedTieredStorages) {
            Send(Owner, new NTiers::TEvTieredStorageResolutionFailed(tieredStorageId, NTiers::TBaseEvObjectResolutionFailed::LOOKUP_ERROR));
        }
        WatchedTieredStorages.clear();
    }
};
